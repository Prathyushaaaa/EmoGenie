...
def moredetails(request):
	...
	if t=='t':
		...
		d = content.objects.filter(Q(category__icontains=r)|Q(category__icontains=r2)).filter(d_type='image').all()
		...
		d = content.objects.filter(Q(category__icontains=r)|Q(category__icontains=r2)).filter(d_type='music').all()
		m=[]
		for d1 in d:
			d1=d1.data
			m.append(d1)
		...
		d = content.objects.filter(Q(category__icontains=r)|Q(category__icontains=r2)).filter(d_type='video').all()
